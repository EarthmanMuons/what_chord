"""Compare Python and pure-Dart bound onset cue records."""

from __future__ import annotations

import argparse
import hashlib
import json
import platform
import subprocess
import sys
from pathlib import Path

import candidate_instance_binding_equivalence
import frame_replay
import onset_evidence
import onset_support
import register_candidates

OUTPUT_SCHEMA = "polychord-onset-cue-record-equivalence/1"
CUE_ID = onset_support.ABLATION_ID
EVIDENCE_SCHEMA_ID = onset_evidence.OUTPUT_SCHEMA
REPO_ROOT = Path(__file__).parents[2]
HARNESS_PATH = Path(__file__).relative_to(REPO_ROOT)
MANIFEST_PATH = Path("research/polychord/data/frame-replay/manifest.json")
OUTPUT_CONTRACT_PATH = Path("research/polychord/automatic-output-contract-v2.md")
ONSET_SUPPORT_CONTRACT_PATH = Path("research/polychord/onset-support-ablation.md")
DART_BATCH_PATH = Path("tool/polychord/onset_cue_record_batch.dart")
DART_HARNESS_TEST_PATH = Path("tool/polychord/onset_cue_record_equivalence_test.py")
DART_PUBLIC_API_PATH = Path("packages/whatchord/lib/whatchord.dart")
DART_MODEL_PATH = Path(
    "packages/whatchord/lib/src/polychord/models/polychord_onset_cue_record.dart"
)
DART_BUILDER_PATH = Path(
    "packages/whatchord/lib/src/polychord/services/"
    "polychord_onset_cue_record_builder.dart"
)
DART_TEST_PATH = Path("packages/whatchord/test/polychord_onset_cue_record_test.dart")


def sha256_file(path: Path) -> str:
    return hashlib.sha256(path.read_bytes()).hexdigest()


def git_output(*args: str) -> str:
    return subprocess.run(
        ("git", *args),
        cwd=REPO_ROOT,
        check=True,
        capture_output=True,
        text=True,
    ).stdout.strip()


def dart_version() -> str:
    result = subprocess.run(
        ("dart", "--version"),
        cwd=REPO_ROOT,
        check=True,
        capture_output=True,
        text=True,
    )
    return (result.stdout or result.stderr).strip()


def observation_frame(*, tracker_epoch: int, frame: dict, notes: list[dict]) -> dict:
    """Serialize the shared Python observation in the Dart frame shape."""

    pressed = [note["midiNote"] for note in notes if note["soundingState"] == "pressed"]
    sustained = [
        note["midiNote"] for note in notes if note["soundingState"] == "sustained"
    ]
    return {
        "trackerEpoch": tracker_epoch,
        "afterEventIndex": frame["afterEventIndex"],
        "timestampMs": frame["timestampMs"],
        "pressedMidiNotes": pressed,
        "sustainedMidiNotes": sustained,
        "soundingMidiNotes": [note["midiNote"] for note in notes],
        "pedalDown": frame["pedalDown"],
        "onsetNotes": notes,
    }


def cue_record(*, tracker_epoch: int, observation: dict, candidate_item: dict) -> dict:
    """Build the frozen onset diagnostic in the v2 cue-record shape."""

    interpretation = onset_support.interpret_onset_evidence(
        candidate_item["onsetEvidence"]
    )
    binding = candidate_instance_binding_equivalence.binding_from_python_opportunity(
        candidate_item, tracker_epoch
    )
    complete = (
        interpretation["availability"] == "complete"
        and binding["availability"] == "complete"
    )
    if (interpretation["availability"] == "complete") != (
        binding["availability"] == "complete"
    ):
        raise ValueError("onset interpretation and binding availability differ")
    return {
        "cueId": CUE_ID,
        "evidenceSchemaId": EVIDENCE_SCHEMA_ID,
        "targetObservation": observation,
        "targetBinding": binding,
        "availability": "complete" if complete else "incomplete",
        "support": interpretation["onsetCohortSupport"] if complete else None,
        "reasonCodes": interpretation["reasonCodes"],
        "diagnostic": {
            "candidate": candidate_item["candidate"],
            "onsetEvidence": candidate_item["onsetEvidence"],
            "onsetInterpretation": interpretation,
        },
    }


def equivalence_cases() -> list[dict]:
    """Build records for all pinned frames and one incomplete control."""

    manifest = frame_replay.load_json(REPO_ROOT / MANIFEST_PATH)
    fixture_paths = frame_replay.validate_manifest(REPO_ROOT / MANIFEST_PATH)
    expected_ids = [entry["id"] for entry in manifest["fixtures"]]
    cases = []
    for expected_id, fixture_path in zip(expected_ids, fixture_paths, strict=True):
        fixture = frame_replay.load_json(fixture_path)
        onset_frames = onset_evidence.replay_onset_frames(fixture)
        for frame, onset_frame in zip(fixture["frames"], onset_frames, strict=True):
            notes = [note.as_dict() for note in onset_frame.notes]
            observation = observation_frame(
                tracker_epoch=0,
                frame=frame,
                notes=notes,
            )
            items = [
                onset_evidence.candidate_onset_evidence(candidate, onset_frame)
                for candidate in register_candidates.generate_register_candidates(
                    frame["soundingMidiNotes"]
                )
            ]
            cases.append(
                {
                    "id": f"{expected_id}/{frame['afterEventIndex']}",
                    "fixtureId": expected_id,
                    "afterEventIndex": frame["afterEventIndex"],
                    "frame": observation,
                    "cueRecords": [
                        cue_record(
                            tracker_epoch=0,
                            observation=observation,
                            candidate_item=item,
                        )
                        for item in items
                    ],
                }
            )

    notes = [
        {
            "midiNote": midi_note,
            "soundingState": "pressed",
            "onsetEventIndex": None,
            "onsetTimestampMs": None,
            "onsetVelocity": None,
        }
        for midi_note in (43, 46, 50, 60, 64, 67)
    ]
    frame = {
        "afterEventIndex": 0,
        "timestampMs": 0,
        "pedalDown": False,
        "soundingMidiNotes": [note["midiNote"] for note in notes],
    }
    onset_frame = onset_evidence.OnsetFrame(
        after_event_index=0,
        timestamp_ms=0,
        notes=tuple(
            onset_evidence.SoundingNoteOnset(
                midi_note=note["midiNote"],
                sounding_state="pressed",
                origin=None,
            )
            for note in notes
        ),
    )
    observation = observation_frame(tracker_epoch=7, frame=frame, notes=notes)
    items = [
        onset_evidence.candidate_onset_evidence(candidate, onset_frame)
        for candidate in register_candidates.generate_register_candidates(
            frame["soundingMidiNotes"]
        )
    ]
    cases.append(
        {
            "id": "synthetic-carried-in/0",
            "fixtureId": None,
            "afterEventIndex": 0,
            "frame": observation,
            "cueRecords": [
                cue_record(
                    tracker_epoch=7,
                    observation=observation,
                    candidate_item=item,
                )
                for item in items
            ],
        }
    )
    return cases


def measurement_payload() -> dict:
    cases = equivalence_cases()
    process = subprocess.Popen(
        ("dart", "run", str(DART_BATCH_PATH)),
        cwd=REPO_ROOT,
        stdin=subprocess.PIPE,
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
        text=True,
        bufsize=1,
    )
    if process.stdin is None or process.stdout is None or process.stderr is None:
        raise RuntimeError("failed to open Dart batch process pipes")

    mismatches = []
    record_count = 0
    complete_count = 0
    incomplete_count = 0
    positive_count = 0
    neutral_count = 0
    try:
        for case in cases:
            process.stdin.write(
                json.dumps(
                    {"id": case["id"], "frame": case["frame"]},
                    separators=(",", ":"),
                )
                + "\n"
            )
            process.stdin.flush()
            response_line = process.stdout.readline()
            if not response_line:
                stderr = process.stderr.read()
                raise RuntimeError(f"Dart batch ended before {case['id']}: {stderr}")
            response = json.loads(response_line)
            if response.get("id") != case["id"]:
                raise ValueError(f"Dart response id differs for {case['id']}")

            expected = case["cueRecords"]
            actual = response["cueRecords"]
            record_count += len(expected)
            complete_count += sum(
                item["availability"] == "complete" for item in expected
            )
            incomplete_count += sum(
                item["availability"] == "incomplete" for item in expected
            )
            positive_count += sum(item["support"] == "positive" for item in expected)
            neutral_count += sum(item["support"] == "neutral" for item in expected)
            if actual != expected:
                mismatches.append(
                    {
                        "caseId": case["id"],
                        "fixtureId": case["fixtureId"],
                        "afterEventIndex": case["afterEventIndex"],
                        "frame": case["frame"],
                        "python": expected,
                        "dart": actual,
                    }
                )
        process.stdin.close()
        return_code = process.wait()
        stderr = process.stderr.read()
        if return_code != 0:
            raise RuntimeError(f"Dart batch failed ({return_code}): {stderr}")
        unexpected_stderr = stderr.replace("Running build hooks...", "").strip()
        if unexpected_stderr:
            raise RuntimeError(
                f"Dart batch wrote unexpected stderr: {unexpected_stderr}"
            )
    finally:
        if process.poll() is None:
            process.kill()
            process.wait()

    return {
        "method": {
            "fixtureManifestSchema": "polychord-frame-replay-manifest/1",
            "outputContract": "polychord-output/2",
            "cueId": CUE_ID,
            "evidenceSchemaId": EVIDENCE_SCHEMA_ID,
            "syntheticControl": (
                "one structural candidate with six carried-in onset "
                "identifiers and nonzero tracker epoch"
            ),
            "comparison": "decoded complete onset cue-record list equality",
            "caseOrder": "manifest fixture order followed by synthetic control",
        },
        "summary": {
            "fixtureCount": len(
                {case["fixtureId"] for case in cases if case["fixtureId"] is not None}
            ),
            "frameComparisonCount": len(cases),
            "pinnedFrameComparisonCount": sum(
                case["fixtureId"] is not None for case in cases
            ),
            "syntheticFrameComparisonCount": sum(
                case["fixtureId"] is None for case in cases
            ),
            "cueRecordComparisonCount": record_count,
            "completeRecordCount": complete_count,
            "incompleteRecordCount": incomplete_count,
            "positiveSupportCount": positive_count,
            "neutralSupportCount": neutral_count,
            "nullSupportCount": record_count - positive_count - neutral_count,
            "mismatchCount": len(mismatches),
        },
        "mismatches": mismatches,
    }


def build_report(command: list[str]) -> dict:
    payload = measurement_payload()
    paths = {
        "fixtureManifest": MANIFEST_PATH,
        "outputContract": OUTPUT_CONTRACT_PATH,
        "onsetSupportContract": ONSET_SUPPORT_CONTRACT_PATH,
        "pythonReplay": Path(frame_replay.__file__).relative_to(REPO_ROOT),
        "pythonGenerator": Path(register_candidates.__file__).relative_to(REPO_ROOT),
        "pythonEvidence": Path(onset_evidence.__file__).relative_to(REPO_ROOT),
        "pythonSupport": Path(onset_support.__file__).relative_to(REPO_ROOT),
        "pythonBindingAdapter": Path(
            candidate_instance_binding_equivalence.__file__
        ).relative_to(REPO_ROOT),
        "dartPublicApi": DART_PUBLIC_API_PATH,
        "dartModel": DART_MODEL_PATH,
        "dartBuilder": DART_BUILDER_PATH,
        "dartTests": DART_TEST_PATH,
        "dartBatch": DART_BATCH_PATH,
        "harness": HARNESS_PATH,
        "harnessTests": DART_HARNESS_TEST_PATH,
    }
    return {
        "schema": OUTPUT_SCHEMA,
        "source": {
            "command": command,
            "workingDirectory": str(Path.cwd().resolve()),
            "repositoryCommit": git_output("rev-parse", "HEAD"),
            "repositoryDirty": bool(git_output("status", "--porcelain")),
            "pythonVersion": platform.python_version(),
            "dartVersion": dart_version(),
            "artifacts": {
                name: {"path": str(path), "sha256": sha256_file(REPO_ROOT / path)}
                for name, path in paths.items()
            },
        },
        **payload,
    }


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--out", required=True, type=Path)
    return parser.parse_args()


def main() -> int:
    args = parse_args()
    report = build_report([sys.executable, *sys.argv])
    args.out.parent.mkdir(parents=True, exist_ok=True)
    args.out.write_text(json.dumps(report, indent=2, sort_keys=True) + "\n")
    summary = report["summary"]
    print(
        f"{summary['frameComparisonCount']} frames and "
        f"{summary['cueRecordComparisonCount']} cue records across "
        f"{summary['fixtureCount']} fixtures; "
        f"{summary['mismatchCount']} mismatches -> {args.out}"
    )
    return int(summary["mismatchCount"] > 0)


if __name__ == "__main__":
    raise SystemExit(main())
