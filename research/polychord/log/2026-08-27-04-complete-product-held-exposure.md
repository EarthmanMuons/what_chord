...
**Goal.** Execute and verify the frozen v3 post-abort reserve completion, inspect
all potentially visible transitions, and make the final product release
decision.
...
`a09a183a7b11dba36c35ded81e696e45007811c06c7031407ffafed2985cf4d0`,
and the v3 output directory was absent. The registered commands ran unchanged:
...
frames it generated 35,301 structural candidate instances on 24,524 frames.
Only two frames were selected and authorized.
...
The complete compact result is
`results/product-held-exposure-v3-summary.json`. The build manifest preserves
the complete per-file and per-source inventory.

**Plain-English reading.** Across all 808 songs, the feature never showed a
polychord annotation. Twice it briefly found a plausible internal candidate,
but the voicing changed after 69 ms, so the stability rule correctly prevented
the candidate from reaching users. The reserve cannot tell us whether real
